...
class Block extends EventEmitter
{
	...
	{
		return this.element
				? Registry.getInstance(this.$el.closest('.' + this.block)[0], this.block, Block)
				: false;
	}
	...
	constructor(node, parms, name)
	{
		...
		if (name.indexOf(this.self.config().dividers.elem)
			&& (tmp = name.split(this.self.config().dividers.elem)).length > 1
		) {
			// Это элемент
			this.block   = tmp[0];
			this.element = tmp[1];
	...
	elems(name)
	{
		...
		let ret = this.self.makeCollection();

		$items.toArray().forEach((node) => {
			ret.push(
				Registry.getInstance(node, s.substr(1), Block)
			);
		});
